...
export async function BattlePageView({ slug, ctx }: { slug: string; ctx: SiteContext }) {
  ...
  const catToQuestion = (name: string) => {
    ...
    if (n.includes("focus") || n.includes("simplic")) return `Which is more focused on ED treatment, ${p1.name} or ${p2.name}?`;
    if (n.includes("range") || n.includes("beyond")) return `Does ${p2.name} offer more than ED treatment compared to ${p1.name}?`;
