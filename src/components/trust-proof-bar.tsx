...
export function TrustProofBar({ config }: { config: SiteConfig }) {
  ...
  return (
    <div className="mb-8 w-full overflow-hidden rounded-2xl border border-[#111111]/15 bg-gradient-to-br from-[#F4F8FB] to-white shadow-[0_1px_3px_rgba(12,75,117,0.06)] sm:max-w-[560px]">
      {/* Big-number row - real magnitudes only */}
      <div className="grid divide-x divide-[#111111]/10" style={{ gridTemplateColumns: gridCols }}>
        {stats.map((s) => (
          <div key={s.label} className="flex flex-col items-center justify-center px-2 py-4 text-center sm:py-5">
            <span className="text-[22px] font-extrabold leading-none tracking-[-0.02em] text-[#111111] sm:text-[28px]">
              {s.value}
            </span>
      ...
      <div className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 border-t border-[#111111]/10 bg-white/60 px-4 py-2.5 sm:px-5">
        ...
        <span className="inline-flex items-center gap-1.5 text-[11.5px] font-medium text-gray-500 sm:text-[12px]">
          <svg viewBox="0 0 20 20" className="h-3.5 w-3.5 text-[#111111]" fill="currentColor" aria-hidden="true">
            <path
              fillRule="evenodd"
